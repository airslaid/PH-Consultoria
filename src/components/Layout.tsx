...
export const Layout: React.FC = () => {
  ...
  const menu = isAdmin ? [
    ...
    { name: 'Demandas', path: '/demandas', icon: <MessageSquare size={16} /> },
    ...
    <div className="flex" style={{ height: '100vh', width: '100vw' }}>
      <div style={{ width: '210px', backgroundColor: '#f9fafb', borderRight: '1px solid var(--border-color)', padding: '1rem', display: 'flex', flexDirection: 'column', zIndex: 10 }}>
        ...
        <div style={{ marginTop: 'auto', paddingTop: '1rem', borderTop: '1px solid var(--border-color)', display: 'flex' }}>
          <button onClick={logout} className="btn btn-secondary w-full" style={{ padding: '0.5rem', color: 'var(--danger)', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', fontSize: '0.8125rem' }} title="Sair do sistema">
            <LogOut size={16} /> Sair
          </button>
        </div>
      </div>
